...
export function CreateClinicForm() {
  ...
  const isValidImageUrl = (url: string): boolean => {
    ...
    try {
      ...
      // Check if it's a valid URL and has proper protocol
      if (protocol !== 'http:' && protocol !== 'https:') return false;
      
      // Check if it's a valid image extension or common image hosting domains
      const validExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg'];
      const validDomains = ['imgur.com', 'cloudinary.com', 'unsplash.com', 'pexels.com', 'images.unsplash.com'];
      
      const hasValidExtension = validExtensions.some(ext => extension.includes(ext));
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    if (manualForm.thumbnail && !isValidImageUrl(manualForm.thumbnail)) {
      setMessage({ type: 'error', text: 'Please enter a valid image URL for the thumbnail' });
      return;
    }
          ...
          <form onSubmit={handleManualSubmit} className="p-6 space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Clinic Photo/Thumbnail (Optional)
                  </label>
                  <input
                    type="url"
                    value={manualForm.thumbnail}
                    onChange={(e) => handleManualInputChange('thumbnail', e.target.value)}
                    placeholder="https://example.com/clinic-photo.jpg"
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                      manualForm.thumbnail && !isValidImageUrl(manualForm.thumbnail) 
                  ...
                  {manualForm.thumbnail && isValidImageUrl(manualForm.thumbnail) && (
                    <div className="mt-2">
                      <p className="text-xs text-green-600 mb-1">✓ Valid image URL</p>
                      <img 
                        src={manualForm.thumbnail} 
                        alt="Clinic thumbnail preview" 
