...
export function AdminSidebar({ activeTab, onTabChange, isOpen, onClose }: AdminSidebarProps) {
  ...
  return (
    <div className={`${isOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0`}>
      {/* Header */}
      <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200 bg-red-600">
        ...
        <ul className="space-y-2">
          {navItems.map((item) => (
            <li key={item.id}>
              ...
              >
                <span className={`mr-3 ${
                  activeTab === item.id ? 'text-red-500' : 'text-gray-400 group-hover:text-gray-500'
                }`}>
                  {item.icon}
                </span>
